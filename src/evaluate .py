...
def plot_ensemble_feature_importance(model, feature_names, top_n=15, save_path=None):
    ...
    if count == 0:
        print("No tree-based models with feature_importances_ found in ensemble.")
        return {}
    ...
    print("\n--- Ensemble Feature Importance (averaged over all models) ---")
    ...
    plt.title(f"Top {top_n} Feature Importances (Ensemble Average)")
    plt.tight_layout()
...
def evaluate_grading_model(model, X_test, y_test, features, model_name):
    ...
    lr_pipeline = model.named_estimators_["lr"]
    X_transformed = lr_pipeline[:-1].transform(X_test)
